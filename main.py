...
from diffusers import AutoencoderKL, UNet2DConditionModel, DDIMScheduler, StableDiffusionPipeline, \
    StableDiffusionImg2ImgPipeline, ControlNetModel, StableDiffusionControlNetPipeline
...
class Pipeline(nn.Module):
    def __init__(self, params):
        ...
        self.unet_pre = deepcopy(self.pipe.unet)
        self.unet_post = deepcopy(self.pipe.unet)

        self.scheduler = DDIMScheduler.from_pretrained(self.model_key, subfolder="scheduler")
        self.scheduler.set_timesteps(params["n_timesteps"], device=params["device"])

        self.controlnet = ControlNetModel.from_pretrained("lllyasviel/sd-controlnet-hed",
    ...
    @torch.no_grad()
    def get_text_embeds(self, positive_prompt, negative_prompt):
        text_input = self.tokenizer(positive_prompt, padding="max_length", max_length=self.tokenizer.model_max_length,
                                    truncation=True, return_tensors="pt")
        text_embeddings = self.text_encoder(text_input.input_ids.to(self.params["device"]))[0]
        uncond_input = self.tokenizer(negative_prompt, padding="max_length", max_length=self.tokenizer.model_max_length,
                                      return_tensors="pt")
        uncond_embeddings = self.text_encoder(uncond_input.input_ids.to(self.params["device"]))[0]
    ...
    def load_img(self, image_path):
        ...
        current_avg = np.mean(lora_image)
        target_avg = 40.0
        scale_factor = target_avg / current_avg
        if current_avg < target_avg:
            lora_image = (lora_image * scale_factor).clip(0, 255).astype(np.uint8)
        lora_image = Image.fromarray(lora_image).resize((512, 512))
        lora_image = np.array(lora_image)

        image_np = lora_image.copy()
        input_tensor = T.ToTensor()(image_np).unsqueeze(0).to(self.params["device"])

        edge_img = cv2.imread(self.params["data_path"])
        current_avg = np.mean(edge_img)
        target_avg = 70.0
        scale_factor = target_avg / current_avg
        if current_avg < target_avg:
            edge_img = (edge_img * scale_factor).clip(0, 255).astype(np.uint8)
    ...
    def decode_latents(self, latents):
        with torch.autocast(device_type=self.params["device"], dtype=torch.float32):
    ...
    def encode_imgs(self, imgs):
        ...
        return latents
    def ddim_inversion(self, condition, latent, save_path):
        timesteps = reversed(self.scheduler.timesteps)
        with torch.autocast(device_type=self.params["device"], dtype=torch.float32):
            for i, t in enumerate(tqdm(timesteps, desc="DDIM-Inversion")):
                cond_batch = condition.repeat(latent.shape[0], 1, 1)
    ...
    def denoise_step(self, t, x=None):
        ...
        if self.is_preprocess_stage:
            ...
            if t == self.scheduler.timesteps[0].item():
                ...
                latent_model_input = torch.cat((style_latent, style_latent, latent_model_input, latent_model_input),
                                               dim=0)
            else:
                latent_model_input = torch.cat([x[0].unsqueeze(0)] * 2 + [x[1].unsqueeze(0)] * 2)

            text_embed_input = self.get_text_embeds(self.params["prompt"], self.params["negative_prompt"])
            ...
            controlnet_noise_pred_uncond, controlnet_noise_pred_cond, noise_pred_uncond, noise_pred_cond = noise_pred.chunk(
                4)
            noise_pred = noise_pred_uncond + self.params["guidance_scale"] * (noise_pred_cond - noise_pred_uncond)
            controlnet_noise_pred = controlnet_noise_pred_uncond + self.params["guidance_scale"] * (
                        controlnet_noise_pred_cond - controlnet_noise_pred_uncond)

            noise_pred = torch.cat([controlnet_noise_pred] + [noise_pred])
            scheduler_output = self.scheduler.step(noise_pred, t, torch.cat(
                [latent_model_input[0].unsqueeze(0)] + [latent_model_input[2].unsqueeze(0)]))
    ...
    def init_pnp(self):
        ...
        register_LoRA_attention_control_efficient(self, self.qk_injection_timesteps, self.params["save_dir"])
        register_attention_control_efficient(self, self.qk_injection_timesteps, self.params["save_dir"])

    def run_pnp(self, inverted_x=None, original_size=None):
        self.init_pnp()
        reconstructed_input = self.sample_loop(inverted_x)

        file_name = os.path.basename(self.params["data_path"])
        ...
        if self.is_preprocess_stage:
            ...
            draw.text((self.lora_image.width + horizontal_gap + 10, reconstructed_image_pil.height + 10),
                      "LoRA reconstructed", font=font, fill="black")

            new_image.save(f"{self.params['save_dir']}/LoRA_Reconstructed/{file_name}")
        else:
            output_image_pil = T.ToPILImage()(reconstructed_input[1])
            output_image_pil.save(f"{self.params['save_dir']}/{file_name}")

            output_image_pil = output_image_pil.resize(original_size)
            resized_dir = os.path.join(self.params["save_dir"], "Resized")
            output_image_pil.save(f"{resized_dir}/{file_name}")
    ...
    def extract_latents(self, step_num, data_path, save_path):
        self.scheduler.set_timesteps(step_num)

        condition = self.get_text_embeds("", "")
        ...
        train_lora(self.lora_image, "", self.model_key, "default", lora_path, self.params["lora_steps"], 0.0005,
                   self.params["lora_batch_size"], 16, -1)
        self.unet_pre.load_attn_procs(lora_path)
        for param in self.unet_pre.parameters():
            param.requires_grad = False

        latent = self.encode_imgs(image)
        inverted_x = self.ddim_inversion(condition, latent, save_path)
...
def run(params):
    ...
    os.makedirs(os.path.join(params["save_dir"], "LoRA_Reconstructed"), exist_ok=True)
    os.makedirs(os.path.join(params["save_dir"], "Resized"), exist_ok=True)

    img_paths = []
    for ext in ["*.png", "*.jpg", "*.jpeg", "*.bmp", "*.PNG", "*.JPG", "*.JPEG", "*.BMP"]:
        img_paths.extend(glob.glob(os.path.join(params["img_dir_path"], ext)))
    ...
    for img_path in img_paths:
        ...
        model.params["data_path"] = img_path
        ...
        model.unet_pre = deepcopy(model.pipe.unet)
        inverted_x = model.extract_latents(
            num_steps=params["n_timesteps"],
            data_path=params["data_path"],
            save_path=params["save_dir"],
        )

        model.is_preprocess_stage = True
        model.run_pnp(inverted_x, original_size)
